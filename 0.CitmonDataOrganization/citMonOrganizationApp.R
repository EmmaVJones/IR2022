...
# Make existing stations layer, this is just BRRO
...
#                DEQ_STA, ASSESSED, STA_TYPE1, STA_TYPE2, STA_TYPE3,DEPTH, 
#                BASIN_NM, VAHU6, REGION,  Longitude, Latitude, everything()) 

#st_write(existingStations, dsn = 'GIS/VEGIS2018IRstations.shp', layer  = 'GIS/VEGIS2018IRstations.shp', 
...
#options(shiny.maxRequestSize=10*1024^2) # not a good idea to override shiny 5mb limit

ui <- fluidPage(
  headerPanel(
    title='Citmon Data Organization'),
...
server <- function(input, output, session){
  ...
  if(is.null(inFile))
    return(NULL)
  read_csv(inFile$datapath, col_types = cols(.default = "c"))  })
  ...
  observeEvent(input$merge_ok, {
    ...
    sites_Merged <- userEnteredStationDataTable() %>% 
      st_drop_geometry() %>%
      mutate(finalStationID = as.character(finalName),
             Reviewer = reactive_objects$reviewer, 
  ...
  Accepted_Data <- reactive({
    if(!is.null(reactive_objects$sites_Accepted)){
      ...
      dplyr::select(reactive_objects$sites_Accepted, UID, originalStationID, finalStationID, Latitude, Longitude) %>%
        mutate(finalStationID = as.character(finalStationID)) %>%
  ...
  output$exp_rev <- downloadHandler(
    ...
    content = function(file) {writexl::write_xlsx(
      list(Accepted_and_Merged_Sites = as.data.frame(rbind(reactive_objects$sites_Accepted,
                                                               reactive_objects$sites_Merged)), # used rbind bc bind_rows was being a pain with column types
