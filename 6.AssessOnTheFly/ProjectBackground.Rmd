...
collectorHeatmap(stationFieldData, stationGIS_View, assessmentLayer, collectorID = 'SMH')
...
```{r}
stationTableResults <- assessmentResults$stationTableResults
    ...
    return(left_join(stationTableResults1, overall2, by = 'STATION_ID') %>%
             mutate(stationColor = case_when(stationOverallScore == 1 ~ 'red',
                    ...
                    `Overall Station Result` = case_when(stationColor == 'red' ~ 'Station contains at least one parameter with 2 or more exceedances',
                                                         ...
                                                         stationColor == 'green' ~ 'Station contains at least one parameter status of S and no exceedances',
                                                         stationColor == 'gray' ~ 'Station unassessed')) %>%
             left_join(parameterSTATcrosswalk, by = c('ParameterEXC' = 'ParameterSTAT')) %>%
             ...
             ### Issue here, need lat/lng info in stationTableResults to get this to work  
...
assessmentSummary <- stationSummary(assessmentResults$stationTableResults, parameterSTATcrosswalk)
```


functions from riverine tool for 30k foot view
```{r}
