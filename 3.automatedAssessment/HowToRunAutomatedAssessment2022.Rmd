...
#i = 1
  ...
  ammoniaAnalysis <- freshwaterNH3limit(stationData, trout = ifelse(unique(stationData$CLASS) %in% c('V','VI'), TRUE, FALSE),
  ...
  if(nrow(stationData) > 0){
    results <- cbind(
      ...
      ammoniaDecision(list(acute = freshwaterNH3Assessment(ammoniaAnalysis, 'acute'),
                           chronic = freshwaterNH3Assessment(ammoniaAnalysis, 'chronic'),
                           fourDay = freshwaterNH3Assessment(ammoniaAnalysis, 'four-day'))), 
      ...
      dplyr::select(-ends_with(c('exceedanceRate', 'VERBOSE', 'Assessment Decision'))) # to match Bulk Upload template but helpful to keep visible til now for testing
