...
VCPMI65results <- pin_get("VCPMI65results", board = "rsconnect") %>%
  filter( between(`Collection Date`, assessmentPeriod[1], assessmentPeriod[2]) ) WCmetals <- pin_get("WCmetals-2020IRfinal",  board = "rsconnect")
...
#i = 1
  ...
  if(nrow(stationData) > 0){
    ...
    results <- filter(stationTable, STATION_ID == stationTable$STATION_ID[i]) %>%
      ...
      mutate(COMMENTS= paste0('This station has no data in current window but was carried over 
                              due to IM in one of the 2020IR status fields. The 2020 comment 
                              was as follows: ', COMMENTS))
  }
