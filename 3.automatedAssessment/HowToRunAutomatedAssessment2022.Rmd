...
#i = 1
  ...
  comments <- stationTableComments(unique(stationData$FDT_STA_ID), stationTable, '2020', historicalStationsTable2, '2018')
  ...
  if(unique(stationData$lakeStation) == TRUE){
    TP <- tibble(NUT_TP_EXC = NA, NUT_TP_SAMP = NA, NUT_TP_STAT = NA) # placeholder for now
  } else {
  ...
  if(unique(stationData$lakeStation) == TRUE){
    chla <- tibble(NUT_CHLA_EXC = NA, NUT_CHLA_SAMP = NA, NUT_CHLA_STAT = NA) # placeholder for now
    } else {
  ...
  if(nrow(stationData) > 0){
    results <- cbind(
      ...
      DO_Assessment_DailyAvg(stationData) %>% quickStats('DO_Daily_Avg'), # run DO daily average status and tuck results into comments later
      ...
      # COMMENTS
      mutate(COMMENTS = paste0('DO_Daily_Avg_STAT: ', DO_Daily_Avg_STAT, 
                               ' | ECOLI Comment: ', ECOLI_STATECOLI_VERBOSE,
                               ' | ENTEROCOCCI Comment: ', ENTER_STATENTER_VERBOSE,
                               ' | AMMONIA Comment: ', `Assessment Decision`)) %>%
      left_join(comments, by = 'STATION_ID') %>%
      dplyr::select(-ends_with(c('exceedanceRate', 'VERBOSE', 'Assessment Decision', 'StationID',
                                 'DO_Daily_Avg_EXC', 'DO_Daily_Avg_SAMP', 'DO_Daily_Avg_STAT'))) # to match Bulk Upload template but helpful to keep visible til now for testing
...
```{r}
stationTableResults <- read_csv('processedStationData/stationTableResults.csv')

lakeStations2 <- mutate(lakeStations, `2020 IR COMMENTS` = NA, `2018 IR COMMENTS` = NA) %>%
  dplyr::select(names(stationTableResults))

stationTableResults <- bind_rows(stationTableResults, lakeStations2)
