...
For Reid: attach WQS information to stations by WQS_ID
```{r WQS join}
riverineWQS <- pin_get('ejones/riverine', board = 'rsconnect') 
FOSR_WQS <- read_csv('Reid/FOSR_WQSlookup.csv') %>% 
  left_join(riverineWQS, by = 'WQS_ID') %>% 
  # add fields that other spatial layers would have brought in
  mutate(Lakes_187B = as.factor(NA),
         WQS_COMMEN= as.factor(NA),
         ZONE = as.factor(NA),
         Shape_Area = as.numeric(NA)) %>% 
  dplyr::select(names(pin_get("WQSlookup-withStandards",  board = "rsconnect")))
...
```{r pull pins}
# May updates
# conventionals <- pin_get("conventionals2022IRfinalWithSecchi-CitmonJames", board = "rsconnect")
# conventionals_distinct <- pin_get("conventionals-distinct-final-CitmonJames", board = "rsconnect")
# stations2020IR <- pin_get("stations2020IR-sf-final-CitmonJames", board = "rsconnect")


# official March Data releases
conventionals <- pin_get("conventionals2022IRfinalWithSecchi", board = "rsconnect")
conventionalsFOSR <- read_csv('Reid/FOSR_conventionals.csv') %>% 
  mutate(FDT_DATE_TIME = as.POSIXct(FDT_DATE_TIME, format = '%m/%d/%Y %H:%M'))
conventionals <- conventionalsFOSR


conventionals_distinct <- pin_get("conventionals-distinct-final", board = "rsconnect")
conventionals_distinctFOSR <- dplyr::select(conventionals, names(conventionals_distinct)) %>% 
  distinct(FDT_STA_ID, .keep_all = T)
conventionals_distinct <- conventionals_distinctFOSR; rm(conventionals_distinctFOSR)
...
stationsTemplate <- read_excel('WQA_CEDS_templates/WQA_Bulk_Station_Upload_Final.xlsx',#'WQA_CEDS_templates/WQA_Bulk_Station_Upload (3).xlsx', 
  ...
  #mutate(ECOLI_IMPAIREDWINDOWS = as.numeric(NA), ENTER_IMPAIREDWINDOWS = as.numeric(NA), `Date Last Sampled` = as.character(NA)) %>% 
#  dplyr::select(STATION_ID:ECOLI_EXC, ECOLI_IMPAIREDWINDOWS, ECOLI_SAMP:ENTER_EXC, ENTER_IMPAIREDWINDOWS, ENTER_STAT:`Date Last Sampled`)
...
stationTableFOSR <- read_csv('Reid/FOSR_stationsTable.csv')
stationTable <- stationTableFOSR
```
...
# make sure all 187 lakes have chla and tp standards
...
# just run what is new based on citmon and may update
citmon <- read_excel('C:/HardDriveBackup/R/GitHub/IR2022/2.organizeMetadata/data/final2022data/Citizen Monitoring Data/20192020CitMonData_20210611.xlsx') %>% distinct(FDT_STA_ID)
conventionals_distinctMarch <- pin_get("conventionals-distinct-final", board = "rsconnect") %>% 
  filter(! FDT_STA_ID %in% conventionals_distinct$FDT_STA_ID) %>% 
  distinct(FDT_STA_ID)
justNew <- bind_rows(citmon, conventionals_distinctMarch) %>% 
  bind_rows(readRDS('data/mayUpdateSites.RDS')) %>% 
  distinct(FDT_STA_ID)

#stationTable <- filter(stationTable, STATION_ID %in% justNew$FDT_STA_ID)
...
```{r}
#write_csv(stationTableResults, 'processedStationData/stationTableResults_CitmonJames.csv',
#          na = "") # dont write out a character NA in csv

# can use write_csv bc there are no date fields that could be corrupted by function
write_csv(stationTableResults, 'Reid/processedStationData/stationTableResultsFOSR.csv',
          na = "") # dont write out a character NA in csv
