...
```{r AU join}
distinctSites_AU <- left_join(distinctSites_sf %>% st_drop_geometry(), 
                              final2020 %>% st_drop_geometry(), # bc two spatial objects need to drop geometry to join on tabular data
                              by = c('FDT_STA_ID' = 'STATION_ID')) %>%
  group_by(FDT_STA_ID) %>%
  mutate(n = n())

# Find any duplicates
View(filter(distinctSites_AU, n >1)) # makes sense, these sites are being used for riverine and lacustrine assessment
```
...
# Needs Work
distinctSites_AUtoDo <- filter(distinctSites_AU, is.na(ID305B_1)) %>%
  st_as_sf(coords = c("Longitude", "Latitude"),  # make spatial layer using these columns
           remove = T, # don't remove these lat/lon cols from df
           crs = 4326)

# All good for assessment app (once we join WQS info from lookup table)
distinctSites_AU <- filter(distinctSites_AU, !is.na(ID305B_1)) %>%
  st_as_sf(coords = c("Longitude", "Latitude"),  # make spatial layer using these columns
           remove = T, # don't remove these lat/lon cols from df
           crs = 4326)
```
...
# Bring in estuary layer
estuaryPolysAU <- st_read('C:/HardDriveBackup/GIS/Assessment/va_2018_aus_estuarine.shp') %>%
   st_transform( 4326 )
