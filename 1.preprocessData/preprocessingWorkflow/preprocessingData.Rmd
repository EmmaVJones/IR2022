...
So we will use this as a pretend dataset to simulate what one could expect from 2022 IR, e.g. some attributed (AU and WQS) stations as well as some new stuff. This exercise will be conducted for BRRO, but the methods will apply statewide.
...
#  select(FDT_STA_ID:FDT_SPG_CODE, Latitude:Data_Source, -FDT_DATE_TIME) # drop data to avoid any confusion 
...
Skip this if you haven't pulled anything from the server recently.

```{r existing WQS_ID information}
...
riverine <- st_read('GIS/WQS_layers_05082020.gdb', layer = 'riverine_05082020' , fid_column_name = "OBJECTID") %>%
  ...
  #st_transform(4326)
#12/7/2020 had to run analysis in NAD_1983_Lambert_Conformal_Conic bc albers was not recognized with new sf
...
```{r data for app}
saveRDS(WQStable, './data/WQStable12072020.RDS')
write.csv(WQStable, './data/WQStable12072020.csv', row.names = F)
