...
```{r citmon data}
citmon <- read_excel('data/CitMonNonAgency/CitMonNonAgencyMonData22IR_test.xlsx') %>%
  ...
  mutate(FDT_DATE_TIME = as.POSIXct(FDT_DATE_TIME, format="%m/%d/%Y %H:%M")) %>%# fix date time
   mutate_at(c('FDT_DEPTH', 'FDT_PERCENT_FRB', 'FDT_FIELD_PH'), as.numeric)
...
together <- bind_rows(conventionals, citmon)
names(conventionals) == names(citmon)
...
If a station snapped to nothing, we will assing it a RL WQS_ID and subbasin it falls into by default.
