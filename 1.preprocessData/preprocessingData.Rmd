...
```{r WQStable}
WQStable <- tibble(StationID = NA, UID = NA)
...
lakesPoly <- st_read('GIS/WQS_layers_05072020.gdb', layer = 'lakes_reservoirs_05072020' , fid_column_name = "OBJECTID") %>%
  mutate(UID = paste0('LP_', as.character(BASIN), "_", sprintf("%06d",as.numeric(as.character(OBJECTID))))) %>% # using OBEJCTID as row number for now
  st_transform(4326)

WQStable <- lakePolygonJoin(lakePoly, distinctSitesToDo, WQStable)
...
riverine <- st_read('GIS/WQS_layers_05072020.gdb', layer = 'riverine_05072020' , fid_column_name = "OBJECTID") %>%
  mutate(UID = paste0('RL_', as.character(BASIN), "_", sprintf("%06d",as.numeric(as.character(OBJECTID))))) %>% # using OBEJCTID as row number for now
  st_transform(102003) # forcing to albers from start bc such a huge layer
  #st_transform(4326)

WQStable <- snapAndOrganizeWQS(distinctSitesToDo[1:50,], 'FDT_STA_ID', riverine, 
...
If a site doesn't attach to a riverine segment, our last step is to try to attach estuary line segments before throwing an empty site to the users for the wild west of manual QA. 
...
WQStable <- snapAndOrganizeWQS(distinctSitesToDo[1:50,], 'FDT_STA_ID', estuarineLines, 
                               bufferDistances = seq(20,80,by=20)) # buffering by 20m from 20 - 80 meters
