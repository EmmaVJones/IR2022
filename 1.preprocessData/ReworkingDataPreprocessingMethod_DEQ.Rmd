...
irData_BRRO <- st_as_sf(irData,
                        ...
                        remove = F, # don't remove these lat/lon cols from df
                        crs = 4326) %>% # add coordinate reference system, needs to be geographic for now bc entering lat/lng, 
  st_intersection(filter(assessmentLayer, ASSESS_REG == "BRRO") %>%
                    filter(Basin == 'James River Basin'))

irData_join <- dplyr::left_join(irData_BRRO, RegionalResultsCombined,
                  by = c('FDT_STA_ID', 'Latitude', 'Longitude','Basin')) 
...
Regional_Sites_AU <- snapAndOrganizeAU_newOutput(irData_join,#irData_missing_NoAU,
                                                 riverineAUs,  
