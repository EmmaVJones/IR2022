...
snapAndOrganizeAU_citmon <- function(Regional_Sites, previousCycleAU, bufferDistances){
  ...
  if(nrow(Regional_Sites_sf_noAU) > 0){
    ...
    snapList_AU <- snap_Points_to_Feature_List(Regional_Sites_sf_noAU,'rowname',
...
#AUsnappedSites <- Regional_Sites_AU_basin
#WQSfileLocation <- 'GIS/updatedWQS/'
#basinName <- basinNameSwitcher(unique(Regional_Sites_AU_basin$Basin))
#bufferDistances <- seq(10,80,by=10)

snapAndOrganizeWQS_citmon <- function(AUsnappedSites, WQSfileLocation, basinName, bufferDistances){
  ...
  snapList_WQS <- snap_Points_to_Feature_List(AUsnappedSites,'rowname',WQS, bufferDistances)
  ...
  if(nrow(snapList_WQS[['sf_output']]) > 0){
    ...
    sites <- filter(snapList_WQS[['sf_output']], !(`Point Unique Identifier` %in% tooMany$`Point Unique Identifier`)) %>%
      st_set_geometry(NULL) %>%
      mutate(rowname=as.numeric(as.character(`Point Unique Identifier`)))
    ...
    if(nrow(tooMany) > 0){
      ...
      siteWithTooMany <- filter(AUsnappedSites, rowname %in% unique(tooMany$`Point Unique Identifier`)) %>%
        st_transform(4326)# project to WGS84 for plotting
  ...
  Sites_AU_WQS <- left_join(AUsnappedSites, results_WQS, by = 'rowname') %>%
    st_set_geometry(NULL)
