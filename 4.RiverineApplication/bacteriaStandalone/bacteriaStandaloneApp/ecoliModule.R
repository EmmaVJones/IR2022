...
# "Conventionals" data
conventionals_HUC1 <- readRDS('data/conventionals_HUC.RDS')
...
server <- function(input,output,session){
  ...
  conventionals_HUC <- reactive({ conventionals_HUC1 })
  ...
  output$stationSelection_ <- renderUI({ req(conventionals_HUC())
    z <- filter(conventionals_HUC(), ID305B_1 %in% input$AUselection) %>%  distinct(FDT_STA_ID)
